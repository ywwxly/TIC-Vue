<template>
  <div class="tic-container__menu-inner">
    <DocumentManagerComponent v-if="!onlyShowLogo" ref="dm-ref"></DocumentManagerComponent>
    <UserListComponent v-if="!onlyShowLogo"  :role="role" :classInfo="classInfo" ref="userlist-ref"></UserListComponent>
    <SettingComponent v-if="!onlyShowLogo" ref="setting-ref"></SettingComponent>
    <ToolboxComponent v-if="!onlyShowLogo" ref="toolbox-ref" @showToolboxDialog="showToolboxDialog"></ToolboxComponent>
    <!-- <ControlAllComponent v-if="!onlyShowLogo" :classInfo="classInfo" ref="control-all-ref"></ControlAllComponent> -->
    <AnswererComponent v-if="!onlyShowLogo" ref="answerer-ref" :members="members"></AnswererComponent>
    <CheckinComponent v-if="!onlyShowLogo" ref="checkin-ref" :members="members"></CheckinComponent>
    ...
    <div v-if="!onlyShowLogo" class="menu-operation">
      <a v-if="role === Constant.CLASS_ROLE.TEACHER" href="javascript:;" class="tic-btn"  @click="setClass" :title="!isClassStarted() ? '上课': '下课'">
        <i :class="['tic-icon', !isClassStarted() ? 'start-class': 'end-class']" ></i>
      </a>
      <a v-if="role===Constant.CLASS_ROLE.STUDENT" href="javascript:;" class="tic-btn"  @click="setHand" :title="!isHandUp() ? '举手': '取消举手'">
        <i :class="['tic-icon', !isHandUp() ? 'hand-up': 'hand-down']"></i>
      </a>
      <a v-if="global.enterprise_id == 100001" href="javascript:;" class="tic-btn mini-program"  @click="getQRCode" title="小程序码">
      </a>
      <ul class="menu-operation__list">
        <li v-for="(item, index) in operations" :key="index">
          <a href="javascript:;" class="tic-btn"  @click="iconTap(item.action)">
            <i :class="`tic-icon tic-icon-${item.class}`" :title="item.title || item.name"></i>
          </a>
        </li>
      </ul>
      <span class="menu-line"></span>
      <div class="menu-user">
          <div :class='["menu-user__img", isMaster?"icon_teacher":"icon_student"]' :style="{'background-image': 'url('+ this.iconurl +')'}"></div>
          <div class="menu-user__text" v-if='this.nickname' v-text='this.nickname'></div>
      </div>
    </div>


    <div class="menu-course">
      <h3 class="menu-course__title">{{ title }}<span class="menu-course__time">{{ hms }}</span></h3>
    </div>
  </div>
</template>
...
export default {
  ...
  props: {
    logo:{
      type: String,
      default: 'https://tic-res-1259648581.file.myqcloud.com/saas/web/tic-logo.png'
    },
    onlyShowLogo: {
      type: Boolean,
      default: false
            ...
            res => {
              if(res.data.error_code === 0 && res.data.user_info) {
                ...
                if (res.data.user_info.avatar && res.data.user_info.avatar.length>0){
                  ...
                  this.iconurl = 'https://main.qcloudimg.com/raw/45e1cd2f9378299a093a6ffa3bb8b9fb.svg'
                }
              }
          })
        }
  ...
  mounted() {
    ...
    this.img = new Image();
    this.img.src = 'http://mp.qcloudtrtc.com/getQRCode?enterprise_id='+(this.Global.enterprise_id || 100001)+'&classId='+this.classInfo.class_id
  ...
  methods: {
    ...
    startTimer() {
      this.timer = setInterval(() => {
        ...
        this.hms = " 已经开课: "+(hour <= 9 ? '0' + hour : hour) + ':' + (min <= 9 ? '0' + min : min) + ':' + (sec <= 9 ? '0' + sec : sec);
      }, 1000);
    },
      ...
      if (this.classInfo.class_status === 'ing') {
        ...
        api.startClass({
          ...
          if(res.data.error_code === 0) {
            ...
            this.$toasted.error(`失败 [${res.data.error_msg} (${res.data.error_code}) ]`)
          }
        })
      }
    ...
    setHand(type) {
      if(type!=undefined && type === "down"){ // 响应IM信令中的hand_down，type="down"
        this.$set(this.classInfo, 'hand_status', 'down')
      }else if ( this.classInfo.hand_status === 'up') {// 学生取消举手，上报hand_down事件
        api.reportEvent({
          class_id: this.classInfo.class_id,
          event: 'hand_down',
        }).then(res => {
      ...
      } else {api.reportEvent({// 学生举手，上报hand_up事件
        class_id: this.classInfo.class_id,
        event: 'hand_up',
      }).then(res => {
    ...
    getQRCode() {
      ...
      this.$Modal.info({
          title: '小程序码',
          content: '<img style="width:85%" src="http://mp.qcloudtrtc.com/qrcode/'+(this.Global.enterprise_id || 100001)+'/' + this.classInfo.class_id + '.png"/>'
      });
    }
